    ...
    typeData.forEach(({ name, damage_relations }) => {
      ...
      const getDmgTypes = dmg => {
        ...
        if (dmgTypes.length<1) {
          ...
          return R.join(', ', dmgTypes)
        }
      }

      const goodAgainst = [getDmgTypes('double_damage_to'), getDmgTypes('half_damage_from'), getDmgTypes('no_damage_from')]
      const formattedGoodAgainst = R.compose(R.join(', '), R.reject(t => t === 'NONE'))(goodAgainst)

      const badAgainst = [getDmgTypes('double_damage_from'), getDmgTypes('half_damage_to'), getDmgTypes('no_damage_to')]
      const formattedBadAgainst = R.compose(R.join(', '), R.reject(t => t === 'NONE'))(badAgainst)

      msg.channel.createMessage(`
        **${name.toUpperCase()} TYPE**
        
        **OFFENSE**:
        **Deals double damage to**: ${getDmgTypes('double_damage_to')}
        **Deals half damage to**: ${getDmgTypes('half_damage_to')}
        **Deals no damage to**: ${getDmgTypes('no_damage_to')}
        
        **DEFENSE**:
        **Takes double damage from**: ${getDmgTypes('double_damage_from')}
        **Takes half damage from**: ${getDmgTypes('half_damage_from')}
        **Takes no damage from**: ${getDmgTypes('no_damage_from')}
        
        **GOOD AGAINST**:
        ${formattedGoodAgainst}
