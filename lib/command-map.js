...
const commandMap = {
    ...
    execute: async (msg, args) => {
      ...
      try {
        ...
        let latestMessage = messages.find(
          (message) => message.member.id === mentionedUserId
        )

        RedisClient.hset(mentionedUserId, 'grab', latestMessage.content)
        console.log(
          `saved ${latestMessage.content} to grab for user-id ${mentionedUserId}`
        )
      } catch (err) {
        throw new Error(err)
      }
    ...
    execute: async (msg, args) => {
      ...
      try {
        ...
        return msg.channel.createMessage(
          `Scheduling reminder "${body}" for ${dateTime} now.`
        )
      } catch (err) {
        throw new Error(err)
      }
  ...
  point: {
    execute: async (msg, args) => {
      ...
      let returnMessage = ''
      try {
        let incrementUser = await msg.channel.guild.members.find(
          (m) => m.id === mentionedTerm
        )
        ...
        let currentPoints = await RedisClient.hget(
          channelId,
          `${mentionedTerm.toLowerCase()}-points`
        )
  ...
  pleaseresetpointsandthankyouforallthatyoudo: {
    execute: async (msg, args) => {
      ...
      try {
        ...
        let clearedUser = msg.channel.guild.members.find(
          (m) => m.id === mentionedUserId
        )
        return msg.channel.createMessage(
          `points reset for ${clearedUser.mention}.`
        )
      } catch (err) {
        throw new Error(err)
      }
  ...
  stopready: {
    execute: async (msg, _args) => {
      ...
      try {
        ...
        return msg.channel.createMessage(
          `If there was a game, it's over now, hope you're happy.`
        )
      } catch (err) {
        throw new Error(err)
      }
    ...
    execute: async (msg, _args) => {
      ...
      try {
        ...
        if (gameStarted !== 'true') {
          ...
          return msg.channel.createMessage(
            `${readyPlayers} out of ${totalPlayers} players ready.`
          )
        }
      } catch (err) {
        throw new Error(err)
    ...
    execute: async (msg, _args) => {
      ...
      try {
        ...
        if (gameStarted !== 'true') {
          return msg.channel.createMessage(stripIndent`
            No game in session.
            Use \`!setready [int]\` to start a ready count for [int] players.
          `)
        } else {
          ...
          if (newCurrentReady >= parseInt(total)) {
            ...
            return msg.channel.createMessage(
              `${newCurrentReady} out of ${total} players ready.`
            )
          }
        }
      } catch (err) {
    ...
    execute: async (msg, args) => {
      try {
        ...
        if (countdown_number > 60) {
          return msg.channel.createMessage(
            `Try not to be an absolute penis. Pick a number of seconds fewer than 60.`
          )
        }
        msg.channel.createMessage(
          `Prepare for COUNTDOWN from ${countdown_number}. Start your engines on GO.`
        )
  ...
  emmyfor: {
    execute: async (msg, args) => {
      ...
      As of ${new Date().toDateString()}, ${args.join(
        ' '
      )} won a massive Emmy. Please applaud.
      `)
    }
  },
  ...
  oscarfor: {
    execute: async (msg, args) => {
      ...
      As of ${new Date().toDateString()}, ${args.join(
        ' '
      )} won a huge Oscar. Please applaud.
      `)
    }
  },
  ...
  grammyfor: {
    execute: async (msg, args) => {
      ...
      As of ${new Date().toDateString()}, ${args.join(
        ' '
      )} won a big Grammy. Please applaud.
      `)
    }
  },
