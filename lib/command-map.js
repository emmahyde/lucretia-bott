  ...
  point: {
    execute: async (msg, args) => {
      ...
      try {
        ...
        let currentPoints = await RedisClient.hget(channelId, `${mentionedTerm}-points`)
        let totalPoints = !currentPoints ? 1 : (parseInt(currentPoints) + 1)
        RedisClient.hset(channelId, `${mentionedTerm}-points`, totalPoints)
    ...
    execute: async (msg, args) => {
      return msg.channel.createMessage(stripIndent`
        ...
        **Happy King Town Features:**
        ...
        \`!setready [num, !sr]\` : Begin a ready count with **num** of people.
