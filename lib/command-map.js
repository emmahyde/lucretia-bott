...
const commandMap = {
  ...
  grab: {
    execute: async (msg, args) => {
      ...
      try {
        ...
        RedisClient.hset(guildId, mentionedUserId, latestMessage.content)
  ...
  lastgrab: {
    execute: async (msg, args) => {
      ...
      let mentionedUserId = args[0].replace(/<@!(.*?)>/, (match, id) => id)
      try {
        let lastGrab = await RedisClient.hget(guildId, mentionedUserId)
  ...
  freakyfriday: {
    execute: async (msg, args) => {
      let guildId = msg.channel.guild.id
      let allStoredGrabs = await RedisClient.hgetall(guildId)
      let arrayIds =  Object.keys(allStoredGrabs)

      let lohan = fortuneCookie(arrayIds.length, arrayIds)
      let lindsay = arrayIds.indexOf(lohan)
      let selflessLove = await RedisClient.hget(guildId, lohan)
          arrayIds.splice(lindsay, 1)
      let jamieleecurtis = fortuneCookie(arrayIds.length, arrayIds)

      // random array object selector
      function fortuneCookie(usersCount, users){
          let randomNumber = Math.floor(Math.random() * usersCount)
          let randomUser = users[randomNumber]
          return randomUser
        }
      try {
        return msg.channel.createMessage(`> ${selflessLove}\n- <@!${jamieleecurtis}>`)
      } catch (err) {
        throw new Error(err)
      }
    }
  ...
  excommunicate: {
    execute: async (msg, args) => {
      ...
      try {
        let incrementUser = await msg.channel.guild.members.find(m => m.id === mentionedTerm)
        if (mentionedTerm === msg.author.id) {
          returnMessage = `${msg.author.mention} has excommunicated themself. Do not disturb!`
        }
        else if (!incrementUser) {
          mentionedTerm = args.join(' ')
          returnMessage = `At the request of ${msg.author.mention}, Pope Urban II has excommunicated ${mentionedTerm} to safeguard the unity of the Church and protect the souls of the faithful. Please avoid ${mentionedTerm}, especially if you are allergic.`
        } else {
          returnMessage = `At the request of ${msg.author.mention} Pope Urban II has excommunicated ${incrementUser.mention}. As a secular realm free of the papacy, its unclear what effect this has. Continue to talk to ${incrementUser.mention} as much or as little as you'd like.`
        }
  ...
  pomodoro: { // pass in number of minutes to wait for
    execute: async (msg, args) => {
      try {
        let pomodoro_timer_length = args[0] || 3
        if (pomodoro_timer_length > 1000) {
          return msg.channel.createMessage(`Too long. I'm tired.`)
        }
        msg.channel.createMessage(`Setting pomodoro timer for ${pomodoro_timer_length}...`)

        setTimeout(() => {
          return msg.channel.createMessage(`YOUR POMODORO IS OVER`)
        }, args[0] * 1000 * 60)
  ...
  titledrop: {
    execute: async (msg, args) => {
      const tdropArray = [
        `https://imgur.com/a/V3yz4VY`,
        `https://imgur.com/a/SzSmxjf`
        // more / better effects to be added
      ]

      const r = Math.floor(Math.random()*tdropArray.length);
      msg.channel.createMessage(tdropArray[r])
  ...
  needthatfic: {
    ...
    } else if (mentionedTerm === msg.author.id) {
      return msg.channel.createMessage(`https://imgur.com/a/c1xDoLi` )
    } else if (incrementUser) {
      return msg.channel.createMessage(`We *all* would just *love* some ${incrementUser.mention} friend fiction, but we don't go saying it in public.` )
    } else if (args.includes('patty')) {
      return msg.channel.createMessage(`https://docs.google.com/document/d/1wISa1QQ1C-Cv7l3hHVYoNpGPaLJIkUskPOR-sc0tFow/edit?usp=sharing` )
  ...
  addto: {
    execute: async (msg, args) => {
      ...
      await RedisClient.rpush(`${guildID}-${listName}`, args.slice(1).join(' '))
      const listItems = await RedisClient.lrange(`${guildID}-${listName}`, 0, -1)
      let list = ''
  ...
  weekpoll: {
    execute: (msg, args) => weekpoll(msg, args)
  },
  ...
  help: {
    execute: async (msg, args) => {
      return msg.channel.createMessage(stripIndent`
        ...
        \`!freakyfriday\` : Oh Mama, stay out of it *please*.
        ...
        \`!titledrop\`: THATS IT! THATS THE NAME!
        \`!needthatfic\`: Participate in the tradition of erotic literature on anyone you please.

        **List Functions:**
        \`!addto [list-name] [list item]\`: Add a **list item** to a **list-name**.
        \`!getlist [list-name]\`: Display item contents of **list-name**.
        ...
        **Happy King Town Features:**
        ...
        \`!pomodoro [num], !pomo [num]\`: Timer for **num** minutes.
        \`!excommunicate [any]\`: *The vile misdeeds have gone on too long. Petition His Holiness and hope he sees reason.*
    `)}
  }
}
...
// One time I tried to do a reminder
